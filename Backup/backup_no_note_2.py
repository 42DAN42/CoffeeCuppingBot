...
class CoffeeCuppingBot:
    ...
    def leave_note(self, update: Update, context: CallbackContext) -> range:
        ...
        if 'note' in self.coffee_ratings[user_id]:
            self.coffee_ratings[user_id]['note'] = note

        if 'total_rating' in context.user_data:
            total_rating = context.user_data['total_rating']
